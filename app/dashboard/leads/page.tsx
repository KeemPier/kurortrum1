    ...
    <main style={{ fontFamily: 'sans-serif', background: '#f8f7f4', minHeight: '100vh' }}>
      <style>{`
        @media (max-width: 768px) {
          ...
          .lead-actions { flex-direction: column !important; align-items: stretch !important; }
        }
      ...
      <nav className="cabinet-nav" style={{ background: 'white', borderBottom: '1px solid #f0f0f0', padding: '0 32px', height: '72px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', boxShadow: '0 1px 8px rgba(0,0,0,0.04)' }}>
        ...
        <div className="cabinet-nav-links" style={{ display: 'flex', alignItems: 'center', gap: '20px' }}>
          ...
          <a className="cabinet-link" href="/dashboard/leads" style={{ color: '#0F4C5C', fontSize: '14px', textDecoration: 'none', fontWeight: 600, whiteSpace: 'nowrap' }}>Заявки</a>
      ...
      <div className="cabinet-pad" style={{ maxWidth: '800px', margin: '0 auto', padding: '32px 24px' }}>
        <h1 style={{ fontSize: '22px', fontWeight: 600, color: '#1a1a1a', marginBottom: '24px' }}>Заявки от туристов</h1>
            ...
            {leads.map(lead => (
              <div key={lead.id} style={{ background: 'white', border: '1px solid #e5e7eb', borderRadius: '12px', padding: '20px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '12px', flexWrap: 'wrap', gap: '8px' }}>
                  <div>
                    <div style={{ fontWeight: 500, fontSize: '16px', marginBottom: '4px', color: '#1a1a1a' }}>{lead.name}</div>
                ...
                <div className="lead-actions" style={{ display: 'flex', gap: '12px', flexWrap: 'wrap' }}>
                  <a href={`tel:${lead.phone}`} style={{ background: '#2BAE8E', color: 'white', padding: '8px 16px', borderRadius: '8px', textDecoration: 'none', fontSize: '14px', textAlign: 'center' }}>
                    📞 {lead.phone}
                  </a>
                  <a href={`https://wa.me/${lead.phone.replace(/\D/g, '')}`} target="_blank" style={{ background: '#25D366', color: 'white', padding: '8px 16px', borderRadius: '8px', textDecoration: 'none', fontSize: '14px', textAlign: 'center' }}>
                    💬 WhatsApp
                  </a>
                </div>
                {lead.message && (
                  <div style={{ marginTop: '12px', padding: '12px', background: '#f8f7f4', borderRadius: '8px', fontSize: '14px', color: '#374151' }}>
                    {lead.message}
                  </div>
                )}
                {lead.need_transfer && (
                  <div style={{ marginTop: '8px', fontSize: '13px', color: '#2BAE8E' }}>🚕 Нужен трансфер</div>
                )}
              </div>
            ))}
          </div>
