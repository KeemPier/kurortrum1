...
- Начиная с записи от 2026-09-04 в репозитории появились Pull Request'ы (до этого их не было — все правки шли прямыми коммитами в ветки).

---

## 2026-09-04 — восстановлена публикация сайта (Cloudflare Pages)

Это не только про код — в этот день была найдена и устранена причина, по которой правки в репозитории `KeemPier/kurortrum1` не попадали на живой сайт `kurortrum.ru`.

**Что было не так:** сайт публикуется через **Cloudflare Pages** (проект `kurortrum1`), а не через Vercel — Vercel в этой цепочке фактически не участвовал. Cloudflare Pages был подключён к **старому** GitHub-репозиторию `bichnguyet4592-a11y/kurortrum1`, в который никто не пушил с 14 июля 2026 (коммит `5307bfe`). Работа же велась в новом репозитории `KeemPier/kurortrum1` — из-за этого автодеплой физически не мог сработать: два разных репозитория, никак не связанных между собой.

**Что сделано (вручную, через панели Cloudflare и Vercel, не через git):**
- В Cloudflare Pages → проект `kurortrum1` → Settings → Git repository: старая связь с `bichnguyet4592-a11y/kurortrum1` отключена, подключён `KeemPier/kurortrum1` (ветка `main`, автодеплой включён).
- Домен `kurortrum.ru` подтверждён и подключён к новому личному аккаунту Vercel (`keempiers-projects`) — как резервная/альтернативная площадка; DNS домена по-прежнему управляется через Cloudflare (`kurortrum.ru` → CNAME → `kurortrum1.pages.dev`), реальный показ сайта посетителям идёт именно через Cloudflare Pages.
- После переподключения Cloudflare Pages автоматически собрал и опубликовал актуальный код (коммит `9833074` — слияние PR #1 с галочкой «Спутниковое ТВ» и этим CHANGELOG) — подтверждено визуально на `kurortrum.ru`.

**Итог:** с этого момента пуш в ветку `main` репозитория `KeemPier/kurortrum1` должен автоматически публиковаться на `kurortrum.ru` через Cloudflare Pages — без ручных шагов.

### Коммиты, связанные с этим днём

- [`9833074`](https://github.com/KeemPier/kurortrum1/commit/98330745af9424111f195b80d890ba51b1839463) — Merge pull request #1 (галочка «Спутниковое ТВ» + первая версия этого CHANGELOG).
